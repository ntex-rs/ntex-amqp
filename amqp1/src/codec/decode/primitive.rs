...
named!(pub null<Null>, map_res!(tag!([0x40u8]), |_| Ok::<Null, ()>(Null)));

named!(pub bool<bool>, alt!(
...
named!(pub ubyte<u8>, do_parse!(tag!([0x50u8]) >> byte: be_u8 >> (byte)));
named!(pub ushort<u16>, do_parse!(tag!([0x60u8]) >> short: be_u16 >> (short)));
named!(pub uint<u32>, alt!(
    do_parse!(tag!([0x70u8]) >> uint: be_u32 >> (uint)) |
    do_parse!(tag!([0x52u8]) >> uint: be_u8 >> (uint as u32)) |
    do_parse!(tag!([0x43u8]) >> (0))
));
named!(pub ulong<u64>, alt!(
    do_parse!(tag!([0x80u8]) >> uint: be_u64 >> (uint)) |
    do_parse!(tag!([0x53u8]) >> uint: be_u8 >> (uint as u64)) |
    do_parse!(tag!([0x44u8]) >> (0))
));

named!(pub byte<i8>, do_parse!(tag!([0x51u8]) >> byte: be_i8 >> (byte)));
named!(pub short<i16>, do_parse!(tag!([0x61u8]) >> short: be_i16 >> (short)));
named!(pub int<i32>, alt!(
    do_parse!(tag!([0x71u8]) >> int: be_i32 >> (int)) |
    do_parse!(tag!([0x54u8]) >> int: be_i8 >> (int as i32))
));
named!(pub long<i64>, alt!(
    do_parse!(tag!([0x81u8]) >> long: be_i64 >> (long)) |
    do_parse!(tag!([0x55u8]) >> long: be_i8 >> (long as i64))
));

named!(pub float<f32>, do_parse!(tag!([0x72u8]) >> float: be_f32 >> (float)));
named!(pub double<f64>, do_parse!(tag!([0x82u8]) >> double: be_f64 >> (double)));

named!(pub char<char>, map_opt!(do_parse!(tag!([0x73u8]) >> int: be_u32 >> (int)), |c| char::from_u32(c)));

named!(pub timestamp<DateTime<Utc>>, do_parse!(tag!([0x83u8]) >> timestamp: be_i64 >> (datetime_from_millis(timestamp))));

named!(pub uuid<Uuid>, do_parse!(tag!([0x98u8]) >> uuid: map_res!(take!(16), Uuid::from_bytes) >> (uuid)));

named!(pub binary<Bytes>, alt!(
...
mod tests {
    ...
    fn test_null() {
        ...
        Null.encode(&mut b);
        let t = null(&mut b).to_full_result();
        assert_eq!(Ok(Null), t);
    }
    ...
    fn test_bool_true() {
        ...
        b1.put_u8(0x41);
        assert_eq!(Ok(true), bool(b1).to_full_result());

        let b2 = &mut BytesMut::with_capacity(0);
        b2.put_u8(0x56);
        b2.put_u8(0x01);
        assert_eq!(Ok(true), bool(b2).to_full_result());
    }
    ...
    fn test_bool_false() {
        let b1 = &mut BytesMut::with_capacity(0);
        b1.put_u8(0x42u8);
        assert_eq!(Ok(false), bool(b1).to_full_result());

        let b2 = &mut BytesMut::with_capacity(0);
        b2.put_u8(0x56);
        b2.put_u8(0x00);
        assert_eq!(Ok(false), bool(b2).to_full_result());
    }
    ...
    fn test_ubyte() {
        let b1 = &mut BytesMut::with_capacity(0);
        (255 as u8).encode(b1);
        assert_eq!(Ok(255 as u8), ubyte(b1).to_full_result());
    }
    ...
    fn test_ushort() {
        ...
        assert_eq!(Ok(350 as u16), ushort(b1).to_full_result());
    }
    ...
    fn test_uint() {
        let b1 = &mut BytesMut::with_capacity(0);
        (0 as u32).encode(b1);
        assert_eq!(Ok(0 as u32), uint(b1).to_full_result());

        let b2 = &mut BytesMut::with_capacity(0);
        (128 as u32).encode(b2);
        assert_eq!(Ok(128 as u32), uint(b2).to_full_result());

        let b3 = &mut BytesMut::with_capacity(0);
        (2147483647 as u32).encode(b3);
        assert_eq!(Ok(2147483647 as u32), uint(b3).to_full_result());
    }
    ...
    fn test_ulong() {
        let b1 = &mut BytesMut::with_capacity(0);
        (0 as u64).encode(b1);
        assert_eq!(Ok(0 as u64), ulong(b1).to_full_result());

        let b2 = &mut BytesMut::with_capacity(0);
        (128 as u64).encode(b2);
        assert_eq!(Ok(128 as u64), ulong(b2).to_full_result());

        let b3 = &mut BytesMut::with_capacity(0);
        (2147483649 as u64).encode(b3);
        assert_eq!(Ok(2147483649 as u64), ulong(b3).to_full_result());
    }
    ...
    fn test_byte() {
        let b1 = &mut BytesMut::with_capacity(0);
        (-128 as i8).encode(b1);
        assert_eq!(Ok(-128 as i8), byte(b1).to_full_result());
    }
    ...
    fn test_short() {
        ...
        assert_eq!(Ok(-255 as i16), short(b1).to_full_result());
    }
    ...
    fn test_int() {
        let b1 = &mut BytesMut::with_capacity(0);
        0.encode(b1);
        assert_eq!(Ok(0), int(b1).to_full_result());

        let b2 = &mut BytesMut::with_capacity(0);
        (-50000).encode(b2);
        assert_eq!(Ok(-50000), int(b2).to_full_result());

        let b3 = &mut BytesMut::with_capacity(0);
        (-128).encode(b3);
        assert_eq!(Ok(-128), int(b3).to_full_result());
    }
    ...
    fn test_long() {
        let b1 = &mut BytesMut::with_capacity(0);
        (0 as i64).encode(b1);
        assert_eq!(Ok(0 as i64), long(b1).to_full_result());

        let b2 = &mut BytesMut::with_capacity(0);
        (-2147483647 as i64).encode(b2);
        assert_eq!(Ok(-2147483647 as i64), long(b2).to_full_result());

        let b3 = &mut BytesMut::with_capacity(0);
        (-128 as i64).encode(b3);
        assert_eq!(Ok(-128 as i64), long(b3).to_full_result());
    }
    ...
    fn test_float() {
        ...
        assert_eq!(Ok(1.234 as f32), float(b1).to_full_result());
    }
    ...
    fn test_double() {
        ...
        assert_eq!(Ok(1.234 as f64), double(b1).to_full_result());
    }
    ...
    fn test_char() {
        let b1 = &mut BytesMut::with_capacity(0);
        '💯'.encode(b1);
        assert_eq!(Ok('💯'), char(b1).to_full_result());
    }
    ...
    fn test_timestamp() {
        ...
        assert_eq!(Ok(expected), timestamp(b1).to_full_result());
    }
    ...
    fn test_uuid() {
        ...
        assert_eq!(Ok(expected), uuid(b1).to_full_result());
    }
    ...
    fn test_binary_short() {
        ...
        assert_eq!(Ok(Bytes::from(&expected[..])), binary(b1).to_full_result());
    }
    ...
    fn test_string_long() {
        ...
        assert_eq!(Ok(expected), string(b1).to_full_result());
    }
    ...
    fn test_symbol_short() {
        ...
        assert_eq!(Ok(Symbol::from("Hello there")), symbol(b1).to_full_result());
    }
    ...
    fn test_symbol_long() {
        ...
        assert_eq!(Ok(expected), symbol(b1).to_full_result());
    }
}