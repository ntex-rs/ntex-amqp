    ...
    U: Encoder + Decoder,
{
        ...
        T: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        loop {
            return match state.decode_item() {
                ...
                Ok(None) => {
                    let n = poll_fn(|cx| Pin::new(&mut *io).poll_read_buf(cx, &mut state.read_buf))
                        .await
        ...
        T: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        poll_fn(|cx| state.flush_io(io, cx))
            .await
            .map_err(Either::Right)
    }
}
    ...
    U: Encoder + Decoder,
{
    ...
    pub(crate) fn send(&mut self, item: <U as Encoder>::Item) -> Result<(), <U as Encoder>::Error> {
