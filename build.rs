...
fn generate_from_spec() {
    ...
    let spec = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/codegen/specification.json"));
    let out_dir = env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR is not defined") + "/src/protocol/";
    ...
    codegen.register_helper(
        "snake",
        Box::new(|h: &Helper, _: &Handlebars, rc: &mut RenderContext| -> Result<(), RenderError> {
    ...
    codegen.register_template_string("definitions", template.to_string()).expect("Failed to register template.");
    ...
    fn reformat_file(path: &std::path::Path) {
        ...
        let mut f = OpenOptions::new().read(true).write(true).open(path).expect("failed to open file");
